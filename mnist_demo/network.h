...
private:
    ...
    Loader &loader;
public:
    NetWork(const std::vector<int> &_sizes, Loader &_loader);
    ~NetWork();
...
template <class Loader>
void NetWork<Loader>::initwb(double *** & _weights, double ** & _bias) {
...
template <class Loader>
void NetWork<Loader>::fill0wb(double *** & _weights, double ** & _bias) {
    for (size_t i = 0; i < num_layers-1; ++ i) {
        size_t x = sizes[i];
        size_t y = sizes[i+1];
        for (size_t j = 0; j < y; ++ j) {
...
template <class Loader>
void NetWork<Loader>::add_nabla_delta() {
    for (size_t i = 0; i < num_layers-1; ++ i) {
        size_t x = sizes[i];
        size_t y = sizes[i+1];
        for (size_t j = 0; j < y; ++ j) {
...
template <class Loader>
void NetWork<Loader>::minus_wb(int batch_cnt, double eta) {
    for (size_t i = 0; i < num_layers-1; ++ i) {
        size_t x = sizes[i];
        size_t y = sizes[i+1];
        for (size_t j = 0; j < y; ++ j) {
...
template <class Loader>
NetWork<Loader>::NetWork(const std::vector<int> &_sizes, Loader &_loader)
...
template <class Loader>
std::vector<double> NetWork<Loader>::feedforward(const std::vector<double> & a) {
...
template <class Loader>
void NetWork<Loader>::sigmoid_array(double *arr, size_t size) {
...
template <class Loader>
double NetWork<Loader>::sigmoid(double z) {
    return 1./(1.+exp(-z));
}

template <class Loader>
void NetWork<Loader>::sigmoid_prime_array(double *arr, size_t size) {
    for (size_t i = 0; i < size; ++ i) {
...
template <class Loader>
double NetWork<Loader>::sigmoid_prime(double z) {
    return sigmoid(z)*(1-sigmoid(z));
}

template <class Loader>
void NetWork<Loader>::dot(double **w, const std::vector<double> & t, double *tmpv, size_t x, size_t y) {
    for (size_t j = 0; j < y; ++ j) {
        double sum = 0;
...
template <class Loader>
void NetWork<Loader>::shuffle(double **p_training_data_x, double **p_training_data_y, size_t len) {
...
template <class Loader>
void NetWork<Loader>::update_mini_batch(double **p_training_data_x, double **p_training_data_y, size_t offset, int mini_batch_size, size_t len, double eta) {
...
template <class Loader>
void NetWork<Loader>::backprop(double *px, double *py) {
...
template <class Loader>
void NetWork<Loader>::cost_derivative(double *pa, double *py, double *c_d, size_t size) {
    for (size_t i = 0; i < size; ++ i) {
        c_d[i] = pa[i] - py[i];
    }
}
